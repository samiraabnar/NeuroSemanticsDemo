...
def plot_func(embedded_words, input_words):
    ...
    for e_word, word in zip(embedded_words, input_words):
        ...
        print("fake acts shape", fake_acts[i - 1].shape)

        fig = plt.figure()
        ax = fig.add_subplot("111", projection='3d')
        plot_predicted_brain_activation = np.array(predicted_brain_activation[0][0],copy=True)
        print(" plot predicted shape: ",plot_predicted_brain_activation.shape)
        plot_predicted_brain_activation += 0.3
        plot_predicted_brain_activation[plot_predicted_brain_activation < 0] = 0.0
        indexes = np.where(plot_predicted_brain_activation > 0.4)[0]
        plot_predicted_brain_activation[indexes] *= 1.4
        plot_predicted_brain_activation[plot_predicted_brain_activation > 1] = 1
        high_indexes = np.where(plot_predicted_brain_activation > 0.7)[0]
        print(high_indexes)
        print(plot_predicted_brain_activation.shape)
    ...
    pad_inches = 0)
        ...
        if (word in words):
            ...
            plot_real_activation = real_activation[0] + 0.3
            plot_real_activation[plot_real_activation < 0] = 0
            indexes = np.where(plot_real_activation > 0.4)[0]
            plot_real_activation[indexes] *= 1.4
            plot_real_activation[plot_real_activation > 1] = 1
            high_indexes = np.where(plot_real_activation > 0.7)[0]
            print(np.max(plot_real_activation))
            print(np.mean(plot_real_activation))
            print(np.min(plot_real_activation))
            print(high_indexes)
            ax.scatter(np.asarray(coords, dtype=int)[:, [0]], np.asarray(coords, dtype=int)[:, [1]],
                       np.asarray(coords, dtype=int)[:, [2]],
                       s=1.0, cmap="jet", c=plot_real_activation, alpha=0.9)
            if(len(high_indexes) > 0):
                ax.scatter(np.asarray(coords, dtype=int)[high_indexes, [0]],
                       np.asarray(coords, dtype=int)[high_indexes, [1]],
                       np.asarray(coords, dtype=int)[high_indexes, [2]],
                       s=1.0, cmap="jet", c=plot_real_activation[high_indexes], alpha=1.0)
